 ...
 */
  ...
  case 1: { /* best fit */
    ...
    while (hp != NIL_HOLE && hp->h_base < swap_base) {
      ...
      if (hp->h_len > clicks && min_hole == NIL_HOLE) {
        min_hole = hp;
      }
      if (hp->h_len > clicks && hp->h_len < min_hole->h_len) {
        min_hole = hp;
      }
      prev_ptr = hp;
      hp = hp->h_next;
 ...
 *===========================================================================*/
PUBLIC int do_hole_map(void) {
  /* Fill a buffer of size nbytes with info about current
  *  contents of the free blocks list maintained by the memory
  *  management module (MM) 
  */
    register struct hole *hp = hole_head; /* point to the first hole */
  /* in MM phys_clicks is synonymous with unsigned int */
    phys_clicks buffer[NR_HOLES*2 + 1]; /* it is NR_HOLES*2 + 1
    * because the recived info in buffer should have the following structure:
    * size1, address1, size2, address2, ..., 0 */
    phys_clicks nbytes, npairs = 0, i = 0;
    nbytes = (phys_clicks)(mm_in.m1_i1 / sizeof(phys_clicks)); /* size of buffer in clicks unit */
    if (nbytes % 2 == 0) nbytes--; /* this is necessary to preserve structure of info in buffer */

    /* while hole is not empty and hp is not yet swap_base
    *  and i is not the last position*/
    while (hp != NIL_HOLE && hp->h_base < swap_base && i < (nbytes - 1)) {
        buffer[i++] = hp->h_len; /* put size of hole in the buffer */
        buffer[i++] = hp->h_base; /* put address of hole in the buffer */
        hp = hp->h_next; /* set iterator to next hole */
        npairs++; /* increment number of size-address pairs in buffer */
    }

    buffer[i] = 0; /* last element in buffer has to be 0 */
  /* transfer the contents of buffers between OS memory and application program memory */
    sys_copy(MM_PROC_NR, D, (phys_bytes)buffer, mm_in.m_source, D, (phys_bytes)mm_in.m1_p1, mm_in.m1_i1);
